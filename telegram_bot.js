...
// إذا لم تكن هناك اتصالات صالحة، استخدم اتصال افتراضي
if (connections.length === 0) {
  console.warn('⚠️ لا توجد RPC URLs صالحة، استخدم متغيرات البيئة RPC_URL و RPC_URL2');
  connections.push(new Connection('https://api.mainnet-beta.solana.com', {
    commitment: 'confirmed',
    confirmTransactionInitialTimeout: 60000
  }));
}

// متغير لتوزيع الأحمال
let connectionIndex = 0;

function getConnection() {
  const connection = connections[connectionIndex % connections.length];
  connectionIndex++;
  return connection;
}
...
// استعلام عبر RPC مع توزيع الأحمال
async function rpc(method, params, preferredConnection = null) {
  const fetch = (await import('node-fetch')).default;
  
  // استخدام الاتصال المفضل أو الحصول على واحد جديد
  const connection = preferredConnection || getConnection();
...
async function getTokenAccounts(address) {
  try {
    ...
    // الحصول على اتصال مع توزيع الأحمال
    const connection = getConnection();

    // استخدام RPC مباشرة مع توزيع الأحمال
...
async function scanDerivationPath(path, seed) {
  try {
    ...
    // استخدام اتصالات متعددة لتوزيع الأحمال
    const connection1 = getConnection();
    const connection2 = getConnection();
    
    const [txList, balance] = await Promise.all([
      retryWithBackoff(() => connection1.getSignaturesForAddress(new PublicKey(address), { limit: 1 })),
      retryWithBackoff(() => connection2.getBalance(new PublicKey(address)))
    ]);
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(chatId, 
    ...
    '📊 استخدم /status لمعرفة حالة الخوادم.'
  );
});

// إضافة أمر لفحص حالة الـ RPC
bot.onText(/\/status/, async (msg) => {
  const chatId = msg.chat.id;
  await bot.sendMessage(chatId, '🔍 جاري فحص حالة الخوادم...');
  
  let statusMessage = '📊 حالة الخوادم:\n\n';
  ...
  for (let i = 0; i < connections.length; i++) {
    try {
      ...
      statusMessage += `✅ خادم ${i + 1}: متصل (${responseTime}ms)\n`;
      statusMessage += `🌐 ${connection.rpcEndpoint}\n\n`;
    } catch (error) {
      statusMessage += `❌ خادم ${i + 1}: غير متصل\n`;
      statusMessage += `🌐 ${connections[i].rpcEndpoint}\n`;
      statusMessage += `⚠️ خطأ: ${error.message}\n\n`;
    }
  }
...
async function checkPrivateKey(privateKey, chatId) {
  try {
    ...
    // الحصول على الرصيد مع توزيع الأحمال
    const connection = getConnection();
...
console.log(`🌐 تم تكوين ${connections.length} اتصال RPC:`);

// فحص حالة الاتصالات عند البدء
for (let i = 0; i < connections.length; i++) {
  const connection = connections[i];
  console.log(`  - خادم ${i + 1}: ${connection.rpcEndpoint}`);
  
  connection.getLatestBlockhash()
    .then(() => console.log(`    ✅ متصل بنجاح`))
    .catch(error => console.log(`    ❌ فشل الاتصال: ${error.message}`));
}
